-- A RPC copiava plano_id da matrícula de origem: alunos re-matriculados para o
-- ano seguinte herdavam o plano do ano anterior (ex: "Mensalidade 2026" em 2027),
-- o que geraria cobrança com valor do ano errado. O plano destino passa a ser
-- explícito, como série e turma. NULL mantém "sem plano".
CREATE OR REPLACE FUNCTION rematriculate(
  p_matricula_id  uuid,
  p_serie_dest_id uuid DEFAULT NULL,
  p_turma_dest_id uuid DEFAULT NULL,
  p_plano_dest_id uuid DEFAULT NULL
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_plano_dest_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM planos WHERE id = p_plano_dest_id AND escola_id = v_mat.escola_id
  ) THEN
    RAISE EXCEPTION 'invalid_plano_dest';
  END IF;
  ...
  VALUES (v_mat.aluno_id, v_mat.escola_id, v_next_serie.id, v_turma.id, p_plano_dest_id,
...
GRANT EXECUTE ON FUNCTION rematriculate(uuid, uuid, uuid, uuid) TO authenticated, service_role;

DROP FUNCTION IF EXISTS rematriculate(uuid, uuid, uuid);
